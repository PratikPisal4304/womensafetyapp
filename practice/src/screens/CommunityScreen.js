...
export default function CommunityScreen() {
  // State to track the text typed in the search bar
  const [searchTerm, setSearchTerm] = useState('');

  const handleSearchSubmit = () => {
    console.log('Search submitted:', searchTerm);
    // You can navigate or filter content based on searchTerm here
  };

  return (
    <SafeAreaView style={styles.container} edges={['left', 'right']}>
      {/* We omit 'top' from edges so the pink background extends behind the status bar */}
      ...
      <View style={styles.headerContainer}>
        ...
        <View style={styles.searchBar}>
          ...
          <TextInput
            ...
            onSubmitEditing={handleSearchSubmit} // Called when user hits "Enter" on keyboard
...
const styles = StyleSheet.create({
  ...
  headerContainer: {
    ...
    paddingTop: 70, // Enough space to cover iOS status bar area
    paddingBottom: 40,
    // Add marginBottom to create space between the header and content
    marginBottom: 20,
  },
  headerTitle: {
    fontSize: 24,
